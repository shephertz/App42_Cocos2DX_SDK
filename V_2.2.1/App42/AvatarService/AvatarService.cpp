...
string AvatarService::buildUpdateAvatarRequest(const char* userName, const char* avatarName, const char* newAvatarName, const char* description)
{
    ...
    if (strlen(newAvatarName))
    {
        cJSON_AddStringToObject(avatarJSON, "newAvatarName", newAvatarName);
    }
    
    if (strlen(description))
    {
        cJSON_AddStringToObject(avatarJSON, "newDescription", description);
    }
...
void AvatarService::UpdateAvatar(const char* avatarName, const char* userName, const char* newAvatarName, const char* description, const app42CallBack& pSelector)
{
    App42AvatarResponse *response = new App42AvatarResponse(pSelector);
    ...
    try
    {
        Util::throwExceptionIfStringNullOrBlank(userName, "User Name");
        Util::throwExceptionIfStringNullOrBlank(avatarName, "Avatar Name");
        Util::throwExceptionIfStringNullOrBlank(newAvatarName, "New Avatar Name");
        Util::throwExceptionIfStringNullOrBlank(description, "Description");
        Util::throwExceptionIfCallBackIsNull(pSelector, "Callback");
    }
    catch (App42Exception *e)
    ...
    map<string, string> signParams;
    populateSignParams(signParams);
    string requestBody = buildUpdateAvatarRequest(userName, avatarName, newAvatarName, description);
    signParams["body"] = requestBody;
    string signature = Util::signMap(secretKey, signParams);
...
void AvatarService::DeleteAvatarByName(const char* avatarName, const char* userName, const app42CallBack& pSelector){
    App42AvatarResponse *response = new App42AvatarResponse(pSelector);
     ...
     */
    string resource = "avatar";
    resource.append("/deleteAvatar/userName/");
    resource.append(userName);
    string baseUrl = getBaseUrl(resource);
    baseUrl.append("?");
    string encodedUrl = url_encode(baseUrl);
...
void AvatarService::DeleteAllAvatars(const char* userName, const app42CallBack& pSelector){
    App42AvatarResponse *response = new App42AvatarResponse(pSelector);
     ...
     */
    string resource = "avatar";
    resource.append("/deleteAllAvatars/userName/");
    resource.append(userName);
    string baseUrl = getBaseUrl(resource);
    baseUrl.append("?");
    string encodedUrl = url_encode(baseUrl);
