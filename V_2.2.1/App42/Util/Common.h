...
namespace Util {
    ...
    static string getJSONKeyString(cJSON *json)
    {
        std::string key;
    ...
    static string getValueForKeyFromJSONString(string key, string jsonString)
    {
        cJSON *ptrJson = cJSON_Parse(jsonString.c_str());
        std::string value;
        
        cJSON *json = ptrJson->child;
        while(json != NULL)
        {
            if((json->string!=NULL) && (strcmp(json->string,key.c_str()) == 0))
            {
                if(json->valuestring != NULL)
                {
                    value = json->valuestring;
