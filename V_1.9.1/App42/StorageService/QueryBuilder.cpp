...
Query* QueryBuilder::BuildQuery(const char* key,std::vector<std::string> value,const char* op)
{
    ...
    cJSON *jsonObject = cJSON_CreateObject();
    cJSON_AddStringToObject(jsonObject, "key", key);
    cJSON *valuesArray = getCJsonFromVector(value);
    cJSON_AddItemToObject(jsonObject, "value", valuesArray);
    cJSON_AddStringToObject(jsonObject, "operator", op);
...
Query* QueryBuilder::BuildQuery(const char* key,cJSON *valueArray,const char* op)
{
    try
    {
        Util::throwExceptionIfStringNullOrBlank(key, "Key");
    ...
    cJSON *jsonObject = cJSON_CreateObject();
    cJSON_AddStringToObject(jsonObject, "key", key);
    cJSON_AddItemToObject(jsonObject, "value", valueArray);
    cJSON_AddStringToObject(jsonObject, "operator", op);
...
cJSON* QueryBuilder::getCJsonFromVector(std::vector<std::string> value)
{
    ...
    {
        cJSON *jsonObj = cJSON_CreateString(it->c_str());
